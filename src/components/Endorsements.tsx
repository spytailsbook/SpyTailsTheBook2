...
const endorsements: Endorsement[] = [
  ...
  },
  {
    id: 8,
    quote:
      "Unbelievable yet meticulously verified. Spy Tails takes readers into covert operations you have to read to believe.",
    author: "Endorsement Author 8",
...
export default function Endorsements() {
  ...
  return (
    <section
      ...
      className="relative z-10 w-full bg-secondary-dark/70 border-y border-accent-amber/20 backdrop-blur-sm py-4 md:py-5 px-4 md:px-8 transition-colors"
      onMouseEnter={() => setIsPaused(true)}
      onMouseLeave={() => setIsPaused(false)}
      onTouchStart={() => setIsPaused(true)}
      onTouchEnd={() => setIsPaused(false)}
    >
      <div className="max-w-[1200px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 md:gap-8">
        
        {/* Left Label Tag */}
        <div className="flex items-center gap-2.5 shrink-0 self-start sm:self-center">
          <Quote className="w-4 h-4 text-accent-amber" aria-hidden="true" />
          ...
          <div className="hidden sm:block h-4 w-px bg-white/10 ml-2" />
        </div>

        {/* Center: Blurb with smooth Fade Transition */}
        <div className="flex-1 w-full min-h-[4.75rem] sm:min-h-[3.75rem] flex items-center justify-center text-center sm:text-left relative px-1">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="flex flex-col sm:flex-row sm:items-baseline gap-1.5 sm:gap-3 w-full justify-center sm:justify-start"
            >
              <p className="text-sm md:text-base font-serif italic text-white/95 leading-snug line-clamp-3 sm:line-clamp-2">
                "{current.quote}"
              </p>
              <div className="shrink-0 text-[11px] md:text-xs font-mono text-text-muted">
                {current.author && (
                  <span className="text-accent-amber/90 font-medium">— {current.author}</span>
                )}
                {current.title && (
        ...
        <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
          <div className="flex items-center gap-1.5 mr-1" role="tablist" aria-label="Endorsement indicators">
